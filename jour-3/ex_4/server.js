...
app.post("/shop", (req, res) => {
    if (req.session.user != undefined) {
        db.collection("products").find({}).toArray(function (err, data) {
            if (err) throw err;
            res.status(200).render("shop", { error: "Welcome " + req.session.user + "<h3 style='text-align:center;margin-top:30px;margin-bottom:11px'>All products:</h3>        <p style='font-size: 16px; margin: 0;margin-bottom: 20px;font-weight: 400;'>Click in the title to see the detail", products: data });
        })
    } else {
...
app.post("/add/admin", (req, res) => {
    ...
    console.log(parseInt(req.body.prix))
